...
def values_to_probabilities(values):
    ...
    adjusted_values = [v - min_value for v in values]
    exponentiated_values = np.exp(-np.array(adjusted_values))
...
# LDA model
base_folder = 'lpb2/' # Location of the np arrays
...
#########################################################################################################################
count = 0
base_arr = [0 for i in range(len(names))]
sum_array = base_arr
sum_array1 = base_arr
pi = base_arr
pinn = base_arr
...
while True:
    ...
    for (x, y, w, h) in faces:
        ...
        pix = values_to_probabilities(np.array(pix)/1e+16)
        ...
        prediction = np.argmax(pi)
        confidence = np.max(pi)
        ...
        cv2.putText(frame, f'DR Mahalanobis {names[prediction]} {confidence:.2f}', (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 0, 0), 2)
        cv2.putText(frame, f'DR NN {names[nn_pred]} {prob_nn:.2f}', (x, y - 40), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 0, 0), 2)
